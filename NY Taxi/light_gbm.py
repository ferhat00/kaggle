...
submission.to_csv('taxi_fare_submission_light_gbm.csv',index=False)


from catboost import Pool, CatBoostRegressor

train_pool = Pool(x_train, y_train, cat_features = [5,6,7,8])
test_pool = Pool(x_test, y_test, cat_features = [5,6,7,8])
model = CatBoostRegressor(iterations=2000, depth=6, learning_rate=0.03, loss_function='RMSE')
...
submission.to_csv('taxi_fare_submission_cat_boost_22M.csv',index=False)
